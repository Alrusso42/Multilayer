...
def train(X_train, y_train, X_valid, y_valid, epochs=500, learning_rate=0.01, batch_size=32, hidden1_size=24, hidden2_size=24):
    ...
    for epoch in range(epochs):
        ...
        # Affichage des métriques toutes les 100 epochs
        if epoch % 100 == 0:
            print(f"--- Epoch {epoch + 1}/{epochs} ---")
            print(f"Train Loss: {loss_train:.4f}, Valid Loss: {loss_valid:.4f}")
            print(f"Train Accuracy: {acc_train:.4f}, Valid Accuracy: {acc_valid:.4f}")
...
def main():
    ...
    parser.add_argument('--layer', type=int, nargs='+', default=[24, 24], help="Number of neurons in hidden layers.")
    parser.add_argument('--loss', type=str, default="binaryCrossentropy", choices=["binaryCrossentropy"], help="Loss function to use.")
    ...
    # Entraîner le modèle
    hidden1_size = args.layer[0] if len(args.layer) > 0 else 24
    hidden2_size = args.layer[1] if len(args.layer) > 1 else 24
    train(X_train, y_train, X_valid, y_valid, epochs=args.epochs, learning_rate=args.learning_rate, batch_size=args.batch_size, hidden1_size=hidden1_size, hidden2_size=hidden2_size)
